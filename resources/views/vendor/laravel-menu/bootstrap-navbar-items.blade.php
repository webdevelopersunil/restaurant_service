@foreach($items as $item)
    ...
    <li @lm_attrs($item) @if($item->hasChildren()) @endif @lm_endattrs>
        @if($item->link) <a @lm_attrs($item->link)
            ...
            {!! $item->title !!}
            @if($item->hasChildren())
                <svg class="menu-icon-tab" xmlns="http://www.w3.org/2000/svg" class="svg-icon iq-arrow-right arrow-active" height="14" width="15" fill="none" viewBox="0 0 24 24" stroke="currentColor">
