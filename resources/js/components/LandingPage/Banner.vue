        ...
        <div class="swiper-wrapper">
            <div v-for="(sliderdata,index) in slider" :key="index" class="swiper-slide item-slide"   v-bind:style="{ backgroundImage: 'url(' + sliderdata.slider_image  + ')' }">
                <div class="inner-content">
                    ...
                    <button class="btn btn-primary mt-3"><router-link class="text-white" :to="{name: 'service-detail',params: { service_id: sliderdata.type_id }}">{{__('messages.book_now')}}</router-link></button>
                </div>
            </div>
        </div>
