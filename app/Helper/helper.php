...
function getLatLong($address=null){
    ...
    $json = file_get_contents("https://maps.google.com/maps/api/geocode/json?address=".$address."&sensor=false&key=AIzaSyAINIcfSsLYDjkJ8ge66Vs3RBvyOtOTc74");
...
function getDirectory($type){
    ...
    $file_type = array(
        ...
        'provider_gallery'=>   'providers/gallery/{uuid}',
    );
...
function common_response($message,$status,$status_code,$data){
    return response()->json( [ 'message'=>$message, 'status'=>$status, 'status_code'=>$status_code, 'data'=>$data] );
}
...
function storeMediaFile($model,$file,$name)
{
    if($file) {
        if( !in_array($name, ['service_attachment'])){
            $res = $model->clearMediaCollection($name);
        }
        if (is_array($file)){
            foreach ($file as $key => $value){
                $res = $model->addMedia($value)->toMediaCollection($name);
            }
        }else{
            $res = $model->addMedia($file)->toMediaCollection($name);
        }
    }
...
function updateMediaFile($model,$file,$name)
{
    if($file) {
        if( !in_array($name, ['service_attachment'])){
            $res = $model->clearMediaCollection($name);
        ...
        // if (is_array($file)){
        //     foreach ($file as $key => $value){
        //         $res = $model->updateMedia($value)->toMediaCollection($name);
        //     }
        // }else{
            $res = $model->updateMedia($file, $name)->toMediaCollection($name);
        // }
    }
...
function sendNotification($type,$user,$data){
  ...
  function remove_old_media_file($id, $type){
...
function getFormattedEmailData($data, $field)
{
...
{
    $email_temp = EmailTemplate::where('name', $name)->where('status', '1')->first();
    return $email_temp;
}
